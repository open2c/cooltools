...
def contrast_diags(M, 
    ...
    """
    compute the contrast in M between different sets of pixels
    specified by modality, modality_params or by I, if given
    if 'normalize' is True, contrast is normalized to be in [-1,1]
    returns:
        - contr_diags: contrast in diagonals with offset s=0..len(M)
        - contr: weighted average over all s
    ...
    modality: string, optional:
        ...
        default: "AnyAny_vs_Mixed"
        => see constrast.indicatormat() for valid modalities

    modality_params: list, optional if modality=='AnyAny_vs_mixed' or None:
        parameters required by 'modality'
        => see contrast.indicatormat() for valid modalities and required parameters.
        typical format: [bin_types, Type], where: 
            bin_types: 1d array: 
                types of the bins, e.g. [0,0,1,1,1,0,...]
                Note: the numerical values identifying the types are inconsequential
                      they are only used to classify the loci into groups
            Type: number:
    ...
    contr_diags: 1D numpy array, len=len(M): 
        contrast in diagonals with offsets s=0..len(M)
        
    contr: float:
        weighted mean of contrast_diags:
        np.nansum(contrast*p)/np.nansum(p) with weights (#set1_pixels(s)*#set2_pixels(s))**.5     
        
    I: 2D array:
        the computed (or supplied) indicator matrix
        
    modality_params: list: 
        the computed (or supplied) modality_params
        useful if modality_params was computed here
    ...
    if I is None:
        if modality_params is None:        
            if modality == 'AnyAny_vs_Mixed':
                ...
                raise ValueError("modality_params=None only allowed with modality='AnyAny_vs_Mixed'. For all other modalityes modality_params has to be given (a list). See contrast.indicatormatrix() for valid modalities and required modality_params.")
    ...
    contr_diags, add_info = contrast_diags_indicatormatrix(M, 
                                                           I, 
...
def contrast_diags_indicatormatrix(M, I, ignore_diags=0, exclude_nans_from_paircounts=True, normalize=True, verbose=False):
    """
    Computes, for all upper diagonals, the 'contrast' in M, 
    namely the ratio of average intensities (values) in M in two sets of pixels
    ...
    contr_norm[s] = (conts[s]-1)/(contr[s]+1)
                  = (<diag(M,s)>_set1 - <diag(M,s)>_set2) / (<diag(M,s)>_set1 + <diag(M,s)>_set2)
...
def get_types(v, vals=None):
    ...
    if vals is not None: restrict to types in vals (floats and ints are considered identical)
...
def indicatormat_Segments_vs_Rest(segments, L=None, bad_bins=[]):
    ...
    segments: list of tuples:
        [(s0,e0),(s1,e1), ...]
        where (s,e) are the start and end points of segments (endpoints are exclusive)
